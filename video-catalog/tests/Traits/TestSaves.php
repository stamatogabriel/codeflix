...
trait TestSaves
{
  ...
  protected function assertStore(array $sendData, array $testDatabase, array $testJsonData = null): TestResponse
  {
    ...
    if ($response->status() !== 201) {
      throw new \Exception("Response status must be have 201, given {$response->status()}: \n{$response->content()}");
    }

    $this->assertInDatabase($response, $testDatabase, $testJsonData);
  ...
  protected function assertUpdate(array $sendData, array $testDatabase, array $testJsonData = null): TestResponse
  {
    ...
    if ($response->status() !== 200) {
      throw new \Exception("Response status must be have 200, given {$response->status()}: \n{$response->content()}");
    }
